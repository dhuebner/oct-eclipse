...
/**
 * JFace dialog for form-based authentication. Renders one labeled input field
 * per {@link FormAuthProviderField} and POSTs the collected data (plus the
 * session token) as JSON to the provider endpoint.
 */
public class FormAuthDialog extends TitleAreaDialog {
	...
	public FormAuthDialog(Shell parentShell, String serverUrl, String token, String endpoint, String providerName,
			FormAuthProviderField[] fields) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		for (FormAuthProviderField field : fields) {
			...
			String labelText = (field.label != null && field.label.message != null) ? field.label.message : field.name;
	...
	protected void okPressed() {
		...
		Thread postThread = new Thread(() -> {
			try {
				// Force HTTP/1.1: the default HttpClient can fail against the local
				...
				if (response.statusCode() >= 200 && response.statusCode() < 300) {
					...
					LOG.warning("Form auth POST failed [" + response.statusCode() + "] to: " + postUrl + " — "
							+ response.body());
				}
			} catch (IOException e) {
				LOG.warning("Form auth POST failed: " + e.getMessage());
			} catch (InterruptedException e) {
				LOG.warning("Form auth POST interrupted: " + e.getMessage());
