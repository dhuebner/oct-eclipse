...
public class AuthenticationService {
	 ...
	 * Shows a provider chooser when multiple providers are available, then
	...
	public void authenticate(String serverUrl, String token, AuthMetadata metadata) {
		Display.getDefault().asyncExec(() -> {
			...
			if (metadata.providers.length == 1) {
				...
				List<String> names = Arrays.stream(metadata.providers).map(p -> p.name != null ? p.name : p.type)
				...
				if (chooser.open() == Window.OK && chooser.getFirstResult() != null) {
					...
					AuthProvider selected = Arrays.stream(metadata.providers)
							.filter(p -> selectedName.equals(p.name != null ? p.name : p.type)).findFirst()
							.orElse(null);
					if (selected != null) {
	 ...
	 */
	private void dispatchProvider(Shell shell, String serverUrl, String token, AuthProvider provider,
	...
	/**
	 * Opens the external browser to the web-auth URL (port of handleWebAuth /
	 * Desktop.browse).
	 */
	...
	/**
	 * Stores the auth token after successful authentication and closes any open
	 * browser dialog.
	 */
