 ...
 *
 * <p>Expects an optional command parameter {@value #PARAM_PEER_ID} carrying the
 * peer id to follow.  When the parameter is absent (or matches the peer already
 * being followed) follow mode is stopped.
 *
 * <p>Registered in plugin.xml against command {@code org.eclipse.oct.toggleFollow}.
 * Typically invoked programmatically from the session view with the peer id
 * embedded as a command parameter.
 */
